...
const init = () => {
  ...
  controls.minPolarAngle = Math.radians(20); //orbit 최소 각도
  controls.maxPolarAngle = Math.radians(120); //orbit 최대 각도
  // controls.target.set(0, 10, 0);
  // controls.autoRotate = true; //자동 로테이트
  // controls.autoRotateSpeed = 1; // 로테이트 속도
  controls.enablePan = false; //키보드 눌렀을때 안되도록
  controls.enableZoom = false; //휠 할때 줌 안되도록
  controls.enableDamping = true; // 돌릴때 부드럽게
  {
...
const animate = () => {
  if (keyDownChk) {
    if (keyCode == "ArrowUp" || keyCode == 38) {
      ...
      wheel_front_Group.children.map((item) => {
        item.rotation.x = item.rotation.x + 0.1;
      });
      wheel_back_Group.children.map((item) => {
        item.rotation.x = item.rotation.x + 0.1;
      });
